...
class LinesAnimation:
    ...
    def Finish(self):
        if not self.IsFinished():
            self.__anime_direct_y = 1
            self.__now_pointlist = copy.deepcopy(self.__pointlist)
            self.__now_pointlist_index = len(self.__now_pointlist)-1

    def IsFinished(self):
        if self.__now_pointlist == self.__pointlist: return True
        else: return False
