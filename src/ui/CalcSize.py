...
class CalcSize:
    def __init__(self, ghostleg: Ghostleg, screen: Screen):
        self.__ghostleg = ghostleg
        self.__screen = screen
        # あみだくじの縦線最大数: 7      最小:2
        # あみだくじの横線最大数: 6      最小:0
        # あみだくじの結果最大文字数: 2  最小:1
        print('ghostleg', ghostleg)
    ...
    def Calculate(self): self.__calc_start_point()
...
#        self.__start_point[1] = font_pixcel_min
        self.__start_point[1] = self.__font.size('▼')[1]
        ...
        #中央寄せ（縦線の数に応じて）
        ghostleg_width = self.__line_width + self.__width_interval * (len(self.__ghostleg.Goals) - 1)
        self.__start_point[0] = (self.__screen.Size[0] - ghostleg_width) / 2
